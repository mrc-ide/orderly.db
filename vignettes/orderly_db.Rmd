...
The root `orderly_config.json` configuration specifies the locations of databases (there can be any number), within the plugins section. For example,
...
The values used in the `args` blocks can be environment values (e.g., `password: $DB_PASSWORD`) in which case they will be resolved from the environment before connecting (including from `orderly_envir.json`).  This will be useful for keeping secrets out of source control.
...
Within a report's main script file, you can extract data from the database as:
...
The contents of `orderly_config.json` may contain things like secrets
...
`orderly_config.json` with key-value pairs, such as
...
This is read every time that `orderly_config.json` is read (in
...
Here, we show yaml snippets that would have worked with orderly version 1 and the corresponding R code that you would use with this plugin
